...
1. lemon_fbx: This tool focus on `blender` more than maya, if you use maya use lemon_maya instead.

    Here is some `required` blender fbx export settings:

    ![](https://i.imgur.com/0ikEMll.png)
    - Scale 0.01: Scale in blender is more 100x compare to maya, this setting reduce scale 100 times back.
    - Triangualate Faces: This setting is a must, because League use triangualted mesh.
    - No Leaf Bones: Bone has name endswith "_end" that blender fbx plugin added for some reason, we don't wan those joints.
    ...
    Bonus: This section is related to maya:
    - When import some converted fbx into maya, there can be very wrong rotate animation playing. To fix this simply change all rotate curve to quaternion slerp by `Select rotation curves -> Graph editor -> Change Rotation Interp -> Quaternion Slerp`.
...
2. lemon_maya: 
    ...
    4. Static object:
        ...
        - Export options:
            ...
            - For `scb only`: Should always use `HasLocalOriginLocatorAndPivot` flag to prevent wrong scb position in game.
    5. Mapgeo:
        ...
        - Material:
            - Material names in MAPGEO files or in BIN files are used with `/`, this character can't be used in Maya, so all `/` will be converted to `__` when import, and will be converted back to `/` when export. Example:
