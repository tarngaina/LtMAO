...
def get_weights(bin):
    ...
    animationGraphData = BINHelper.find_item(
        items=bin.entries,
        compare_func=lambda entry: entry.type == PRE_BIN_HASH['animationGraphData']
    )
    ...
    mMaskDataMap = BINHelper.find_item(
        ...
        compare_func=lambda field: field.hash == PRE_BIN_HASH['mMaskDataMap']
    )
    ...
    for mask_name, MaskData in mMaskDataMap.data.items():
        mask_data[mask_name] = BINHelper.find_item(
            items=MaskData.data,
            compare_func=lambda field: field.hash == PRE_BIN_HASH['mWeightList'],
...
def set_weights(bin, mask_data):
    ...
    for mask_name, MaskData in mMaskDataMap.data.items():
        for field in MaskData.data:
            if field.hash == PRE_BIN_HASH['mWeightList']:
